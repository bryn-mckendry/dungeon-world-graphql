...
import { getMonsterSettingById, getMonsterSettings } from '../database/monsterSetting';
...
export const RootQueryType = new GraphQLObjectType({
  ...
  fields: () => ({
    monsterSetting: {
      ...
      resolve: async (_, { id }) => await getMonsterSettingById(id)
    },
    monsterSettings: {
      type: new GraphQLList(MonsterSettingType),
      description: 'List of all monster settings.',
      resolve: async () => await getMonsterSettings()
    },
    monster: {
      type: MonsterType,
